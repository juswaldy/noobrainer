...
top2vec, doc_id_type, has_documents = tomo.load(settings.tomo_model_path)
...
async def query_topics(request: PredictionRequest):
    topics_words, words_scores, topic_scores, topic_nums = top2vec.query_topics(
...
async def search_topics_by_keywords(keyword_search: KeywordSearchTopic):
    topics_words, words_scores, topic_scores, topic_nums = top2vec.search_topics(
...
async def search_documents_by_keywords(keyword_search: KeywordSearchDocument):
    documents = []
    docs, doc_scores, doc_ids = top2vec.search_documents_by_keywords(
        keyword_search.keywords,
...
async def search_documents_by_documents(document_search: DocumentSearch):
    documents = []
    docs, doc_scores, doc_ids = top2vec.search_documents_by_documents(
